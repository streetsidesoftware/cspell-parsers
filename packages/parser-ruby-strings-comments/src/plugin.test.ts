...
describe('plugin', () => {
  it('exposes the ruby-strings-comments parser', () => {
...
describe('customizePlugin', () => {
  it('wires tag filtering into the ruby-strings-comments parser', () => {
  ...
  it('renames the parser with renameParser, and languageSettings follow', () => {
    const customized = customizePlugin().renameParser('ruby-strings-comments', 'custom-example');

    expect(customized.parserNames()).toEqual(['custom-example']);
    expect(customized.languageSettings()).toEqual([{ languageId: 'ruby', parser: 'custom-example' }]);
  });

  it('returns a copy that can be customized further, leaving plugin unchanged', () => {
    const customized = customizePlugin().renameParser('ruby-strings-comments', 'ruby-comments');

    expect(customized.parserNames()).toEqual(['ruby-comments']);
    expect(plugin.parserNames()).toEqual(['ruby-strings-comments']);
  });
});
