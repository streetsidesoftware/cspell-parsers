...
export class Scanner {
  ...
  private *scanCode(end: number, stopAtUnmatchedBrace: boolean): Generator<ParsedText> {
    ...
    while (this.i < end) {
      ...
      if (c === '"' && (!sawSlash || canPrecedeString(content[this.i - 1]))) {
        yield* this.scanInterpolatedString('"', TAGS.STRING_DOUBLE);
        continue;
      }
      if (c === '`') {
        ...
        yield* this.scanInterpolatedString('`', TAGS.STRING_BACKTICK);
        continue;
      }
  ...
  private scanBeginEndComment(): ParsedText {
    ...
    this.i = footerEnd;
    return { text, rawText, map, range: [start, footerEnd], tags: TAGS.COMMENT_BLOCK };
  }
  ...
  private scanSingleQuotedString(): ParsedText {
    ...
    this.i = end;
    return { text, rawText, map, range: [start, end], tags: TAGS.STRING_SINGLE };
  }
   ...
   * interpolation hole.
   */
  private *scanInterpolatedString(quoteChar: string, tags: Tags): Generator<ParsedText> {
    const { content } = this;
    let i = this.i + 1;
    let fragStart = i;
    for (;;) {
  ...
  private *emitFragment(start: number, end: number, tags: Tags): Generator<ParsedText> {
    if (end <= start) return;
  ...
  private *scanHeredocBody(header: HeredocHeader): Generator<ParsedText> {
    ...
    if (!interpolated) {
      yield* this.emitFragment(bodyStart, bodyEnd, TAGS.STRING_HEREDOC);
    } else {
      this.i = bodyStart;
      ...
      while (this.i < bodyEnd) {
        ...
        if (c === '#' && content[this.i + 1] === '{') {
          yield* this.emitFragment(fragStart, this.i, TAGS.STRING_HEREDOC);
          this.i += 2;
      ...
      yield* this.emitFragment(fragStart, bodyEnd, TAGS.STRING_HEREDOC);
    }
    this.i = markerEnd;
  }
