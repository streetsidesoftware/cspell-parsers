...
A lightweight Java parser for [cspell](https://cspell.org) that spell checks the prose in your code: comments
and strings. It has no dependencies, and it gives you control over what gets checked, from Javadoc comments to
text blocks.
...
```jsonc
{
  "import": ["@cspell/parser-java-strings-comments"],
  "languageSettings": [
    {
      "languageId": "java",
...
to change what gets checked. For example, to check only Javadoc comments:
...
> also matches the more specific `string.textBlock`, unless a more specific key overrides it. A key can also use
...
```js
customizePlugin().renameParser('java-strings-comments', 'my-java-parser');
```

## Tags

Each part of a file gets its most specific tag plus the more general ones above it. For example, a Javadoc
