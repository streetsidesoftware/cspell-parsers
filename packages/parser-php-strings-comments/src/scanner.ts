...
/**
 * Scans PHP source for comments and string/heredoc/nowdoc literals (each tagged with its own specific tag),
 * and passes everything else through too - HTML markup as `html`, and any PHP code that isn't a comment or
 * string literal (identifiers, keywords, punctuation, numbers, tag delimiters) as `code` - so every byte of
 * the file ends up in exactly one `ParsedText`.
 *
 * PHP alone among this repo's split-out language families has a genuine two-mode structure: a file toggles
 * between HTML markup (passed through as `html`) and PHP code at `<?php`/`<?=`/`<?` and `?>` boundaries.
 * `run`/`scanPhpDocument` drive that toggle; `scanCode` handles everything inside a PHP region. `run` also
 * fills in the `code`-tagged gaps between what `scanPhpDocument`/`scanCode` themselves yield, via `j` - a
 * second cursor trailing `i`, marking how far the emitted segments have covered so far.
 */
export class Scanner {
  ...
  *run(): Generator<ParsedText> {
    for (const parsed of this.scanPhpDocument()) {
  ...
  /**
   * Alternates between HTML markup (everything up to the next `<?php`/`<?=`/`<?`, tagged `html`) and a PHP
   * code region (handed to `scanCode` in `phpAware` mode, which returns control here as soon as it sees a
   * top-level `?>`, or otherwise runs to the end of the file).
   */
  private *scanPhpDocument(): Generator<ParsedText> {
    ...
    while (this.i < content.length) {
      ...
      if (htmlEnd > this.i) {
        ...
        yield { text, rawText: text, range: [this.i, htmlEnd], tags: TAGS.HTML };
      }
      if (!tag) {
  ...
  private scanLineComment(phpAware: boolean, isHash: boolean): { parsedText: ParsedText; closesPhp: boolean } {
    ...
    return { parsedText: { text, rawText, map, range: [start, end], tags: TAGS.COMMENT_LINE }, closesPhp };
  }
  ...
  private *scanHeredoc(): Generator<ParsedText> {
    ...
    yield {
      ...
      tags: quote === "'" ? TAGS.STRING_NOWDOC : TAGS.STRING_HEREDOC,
    };
  }
}
