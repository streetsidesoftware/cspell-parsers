...
'csharp') ...`) - since this package only ever handles one syntax, there's no dialect type or detection at
all, and no interpolation/template-literal handling either (C/C++ strings never split into fragments), which
is most of why this scanner is much smaller than the combined one.
