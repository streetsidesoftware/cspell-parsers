...
describe('customizePlugin', () => {
  ...
  it('renames the parser with renameParser, and languageSettings follow', () => {
    const customized = customizePlugin().renameParser('c-cpp-strings-comments', 'custom-example');

    expect(customized.parserNames()).toEqual(['custom-example']);
    expect(customized.languageSettings()).toEqual([{ languageId: 'c,cpp', parser: 'custom-example' }]);
  });

  it('returns a copy that can be customized further, leaving plugin unchanged', () => {
    const customized = customizePlugin().renameParser('c-cpp-strings-comments', 'c-comments');

    expect(customized.parserNames()).toEqual(['c-comments']);
    expect(plugin.parserNames()).toEqual(['c-cpp-strings-comments']);
  });
});
