...
describe('plugin', () => {
  it('exposes the typescript-strings-comments parser', () => {
...
describe('customizePlugin', () => {
  it('wires tag filtering into the typescript-strings-comments parser', () => {
  ...
  it('renames the parser with renameParser, and languageSettings follow', () => {
    const customized = customizePlugin().renameParser('typescript-strings-comments', 'custom-example');
  ...
  it('returns a builder that can be customized further, leaving plugin unchanged', () => {
    ...
    expect(customized.parserNames()).toEqual(['ts-comments']);
    expect(plugin.parserNames()).toEqual(['typescript-strings-comments']);
  });
});
