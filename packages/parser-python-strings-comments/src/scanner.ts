 ...
 */
function stringTags(base: Tags, isRaw: boolean, isInterpolated: boolean): Tags {
  if (!isRaw && !isInterpolated) return base;
  return {
    ...base,
    ...(isRaw ? TAGS.STRING_RAW_FLAG : undefined),
    ...(isInterpolated ? TAGS.STRING_INTERPOLATED_FLAG : undefined),
  };
}
...
export class Scanner {
  ...
  private *scanString(prefixLen: number, isRaw: boolean, isF: boolean): Generator<ParsedText> {
    ...
    const baseTag = isTriple ? TAGS.STRING_TRIPLE : quote === "'" ? TAGS.STRING_SINGLE : TAGS.STRING_DOUBLE;
   ...
   */
  private scanPlainStringBody(start: number, openLen: number, quote: string, delimLen: number, tag: Tags): ParsedText {
    const { content } = this;
    let i = start + openLen;
    let closed = false;
