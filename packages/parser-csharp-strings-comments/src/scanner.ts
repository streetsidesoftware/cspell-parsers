...
export class Scanner {
  ...
  private scanLineComment(): ParsedText {
    ...
    this.i = end;
    return { text, rawText, map, range: [start, end], tags: isTripleSlash ? TAGS.COMMENT_LINE_DOC : TAGS.COMMENT_LINE };
  }
  ...
  private scanCSharpVerbatimString(start: number): ParsedText {
    ...
    this.i = end;
    return { text, rawText, map, range: [start, end], tags: TAGS.STRING_VERBATIM };
  }
  ...
  private *scanCSharpInterpolatedString(start: number, verbatim: boolean): Generator<ParsedText> {
    const { content } = this;
    const openLen = this.i - start + 1;
    const tag = verbatim ? TAGS.STRING_VERBATIM_INTERPOLATED : TAGS.STRING_INTERPOLATED;
    let i = start + openLen;
    let fragStart = i;
    for (;;) {
      if (i >= content.length) {
  ...
  private scanCSharpRawString(start: number, quoteRun: number, interpolated: boolean): ParsedText {
    ...
    return {
      ...
      map,
      range: [start, end],
      tags: interpolated ? TAGS.STRING_RAW_INTERPOLATED : TAGS.STRING_RAW,
    };
  }
