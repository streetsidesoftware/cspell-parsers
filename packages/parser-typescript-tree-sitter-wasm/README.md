...
```jsonc
{
  "import": ["@cspell/parser-typescript-tree-sitter-wasm"],
  "languageSettings": [
...
- **Package names aren't spell checked.** The package name in an `import`, `export ... from`, `import(...)`,
  ...
  <!--- Tested by src/parsers.test.ts: "imports.ts" and "imports-and-local-variables.mts" --->
...
- **Names that come from an imported package aren't spell checked.** That includes a named import's original
  ...
  <!--- Tested by src/parsers.test.ts: "imports.ts" and "module-bindings.ts" --->
...
- **A local name that reuses an import's name is checked.** A variable or parameter that shadows an import is
  ...
  <!--- Tested by src/parsers.test.ts: "imports-and-local-variables.mts" and "lets a JavaScript parameter shadow an import of the same name" --->
...
- Uses WebAssembly (via `@vscode/tree-sitter-wasm`), so it doesn't need a native compiler toolchain to
  install.
