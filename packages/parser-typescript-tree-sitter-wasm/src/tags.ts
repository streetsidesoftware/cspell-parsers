...
/**
 * Builds a `Tags` object with every dot-separated ancestor of `tag` set to `true`, in addition to `tag`
 * itself - e.g. `hierarchicalTags('comment.block.doc')` is `{ comment: true, 'comment.block': true,
 * 'comment.block.doc': true }` - so a consumer can filter on any level (`tags.comment` or
 * `tags['comment.block']`) without its own prefix-matching logic. `tag`'s type restricts this to a
 * documented tag; every ancestor prefix of a documented tag is itself documented, by this file's own
 * convention.
 *
