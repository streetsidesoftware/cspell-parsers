...
replaces them with a deliberate model: each package exports an immutable `IPlugin`, and users customize it
through an `IPluginBuilder` to rename, duplicate, add, or remove parsers, change their file types, and filter
their tags. During the migration the new types were called `IPluginEx`/`IParserEx` and lived alongside the
old ones, which were then removed, and the new types took the plain names (see
[0002's amendment](./0002-compatibility-and-migration.md#amendment-the-ex-names-are-folded)).
...
| 0002 | Breaking externally is allowed; the repo is never broken internally                       | Accepted, amended |
...
| 0008 | `customizePlugin` stays as a thin wrapper that returns a builder                          | Accepted, amended |
| 0009 | `defineConfig` merges a plugin into the user's settings                                   | Accepted          |
| 0010 | `filterTagsForFileType` gives a file type its own filtered parser                         | Accepted          |
