...
### 2. Audit a release PR's entries

This repo's release notes cover many packages at once (release-please's `node-workspace` plugin bundles every
`packages/*` release into one PR), so entries are scoped commits like `feat(parser-ruby-strings-comments): ...`
...
`BEGIN_COMMIT_OVERRIDE` only works on squash-merged PRs — Release Please can't tell which commit to apply it to on a plain merge. This repo only allows squash merges (`gh api repos/streetsidesoftware/cspell-parsers --jq .allow_squash_merge` → `true`, and `.allow_merge_commit`/`.allow_rebase_merge` → `false`), so every merged PR here qualifies automatically. If this skill is ever pointed at a different repo, check its merge settings the same way before proceeding.
